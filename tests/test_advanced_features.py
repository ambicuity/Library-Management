...
class TestAdvancedSearch(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.library.add_book(
            Book("Lord of the Rings", "J.R.R. Tolkien", category="Fantasy")
        )
        self.library.add_book(Book("Dune", "Frank Herbert", category="Science Fiction"))
        self.library.add_book(
            Book("Foundation", "Isaac Asimov", category="Science Fiction")
        )
        self.library.add_book(Book("1984", "George Orwell", category="Dystopian"))
        self.library.add_book(
            Book("Animal Farm", "George Orwell", category="Political Fiction")
        )
        self.library.add_book(
...
class TestOverdueTracking(unittest.TestCase):
    ...
    def test_get_overdue_books_multiple_members(self):
        ...
        alice.checked_out_books["Book 1"] = CheckedOutBook(
            "Book 1", "Author 1", past_date_1.isoformat()
        )
        bob.checked_out_books["Book 2"] = CheckedOutBook(
            "Book 2", "Author 2", past_date_2.isoformat()
        )
    ...
    def test_display_overdue_books_with_overdue(self, mock_print):
        ...
        past_date = date.today() - timedelta(days=5)
        alice.checked_out_books["Book 1"] = CheckedOutBook(
            "Book 1", "Author 1", past_date.isoformat()
        )
...
class TestCategoryManagement(unittest.TestCase):
    ...
    def test_display_books_by_category_existing(self, mock_print):
        ...
        self.assertTrue(
            any("Books in 'Fiction' category:" in call for call in call_strings)
        )
        self.assertTrue(any("Book 1" in call for call in call_strings))
...
class TestDatabaseInitializationAndSetup(unittest.TestCase):
    ...
    def test_database_with_existing_files(self):
        ...
        initial_members = [{"name": "Existing Member", "books": ["Existing Book"]}]
    ...
    def test_large_dataset_handling(self):
        ...
        self.assertGreaterEqual(
            len(results), 1
        )  # Should find "Book 1", "Book 10", etc.
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    def test_full_library_workflow(self):
        ...
        self.assertEqual(
            len(new_library.books), 2
        )  # Only available books are persisted
    ...
    def test_overdue_workflow_integration(self):
        ...
        past_date_1 = date.today() - timedelta(days=3)
        member1.checked_out_books["Book 1"] = CheckedOutBook(
            "Book 1", "Author 1", past_date_1.isoformat()
        )

        # Make Book 2 overdue by 7 days
        past_date_2 = date.today() - timedelta(days=7)
        member2.checked_out_books["Book 2"] = CheckedOutBook(
            "Book 2", "Author 2", past_date_2.isoformat()
        )
    ...
    def test_search_and_category_integration(self):
        ...
        space_sci_fi = [
            book for book in space_books if book.category == "Science Fiction"
        ]
...
class TestStressAndPerformance(unittest.TestCase):
    ...
    def test_concurrent_operations_simulation(self):
        ...
        self.assertGreaterEqual(
            total_issued, 25
        )  # At least 25 members should have books
    ...
    def test_massive_20k_book_collection_comprehensive_edge_cases(self):
        ...
        self.assertGreater(
            len(all_categories), 20
        )  # Should have most of our 29 categories
        ...
        overdue_count = 0
        for i in range(
            0, min(1000, issued_count), 2
        ):  # Every other member in first 1000
        ...
        expected_available_books = (
            20000 - issued_count + 1
        )  # +1 for duplicate test book added later
        remaining_books_in_library = len(new_library.books)

        # The actual count might be slightly different due to duplicate handling
        self.assertGreaterEqual(
            remaining_books_in_library, expected_available_books - 100
        )
        ...
        try:
            ...
            if available_books:
                self.library.return_book(
                    available_books[0].title, "Member_00001", available_books[0].author
                )
        except Exception:
            pass  # Expected to fail
        ...
        invalid_search_results.append(
            self.library.search_books(None, "title")
            if hasattr(self.library, "search_books")
            else []
        )
        invalid_search_results.append(
            self.library.search_books("test", "invalid_type")
            if hasattr(self.library, "search_books")
            else []
        )
        ...
        print("🔍 Search operations: Multiple types tested")
        print(f"📂 Categories tested: {len(all_categories)}")
        print("💾 Data persistence: Save and load tested")
        print("⚠️  Edge cases: Comprehensive validation")
        print("⚡ Performance: Validated with rapid operations")
        ...
        self.assertGreaterEqual(
            total_books_in_system, 20000
        )  # Should be around 20,001 (including duplicate)
        self.assertEqual(len(self.library.members), 2000)
        self.assertGreater(
            issued_count, 1400
        )  # Should have issued most books successfully
        self.assertGreater(
            len(overdue_books), 400
        )  # Should have substantial overdue books
        self.assertGreater(
            len(all_categories), 20
        )  # Should have most categories represented

        print("✅ All assertions passed - system handles 20K+ books flawlessly!")
