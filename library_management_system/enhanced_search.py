...
class EnhancedSearch:
    ...
    def fuzzy_search_titles(self, query: str, limit: int = 10) -> List[Tuple[str, int]]:
        ...
        return [(match[0], match[1]) for match in matches if match[1] >= self.fuzzy_threshold]
    
    def fuzzy_search_authors(self, query: str, limit: int = 10) -> List[Tuple[str, int]]:
        ...
        # Use fuzzy matching
        matches = process.extract(query, authors, scorer=fuzz.partial_ratio, limit=limit)
        return [(match[0], match[1]) for match in matches if match[1] >= self.fuzzy_threshold]
    
    def _simple_fuzzy_search_titles(self, query: str, limit: int) -> List[Tuple[str, int]]:
        ...
        for title, score in title_scores.items():
            ...
            if max_trigrams > 0:
                ...
                if similarity >= (self.fuzzy_threshold - 20):  # Lower threshold for fallback
                    results.append((title, similarity))
        
        results.sort(key=lambda x: x[1], reverse=True)
        return results[:limit]
    
    def _simple_fuzzy_search_authors(self, query: str, limit: int) -> List[Tuple[str, int]]:
    ...
    def search_members(self, query: str, fuzzy: bool = False) -> List[Member]:
        ...
        if fuzzy and HAS_FUZZYWUZZY:
            ...
            matches = process.extract(query, member_names, scorer=fuzz.partial_ratio, limit=10)
...
class SearchSuggestionEngine:
    ...
    def get_suggestions(
        ...
        query_lower = query.lower()
        popular = [
            q for q, count in sorted(self.popular_queries.items(), key=lambda x: x[1], reverse=True)
