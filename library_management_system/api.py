...
class LibraryAPI:
    ...
    def _setup_routes(self) -> None:
        ...
        @self.app.post("/users", response_model=UserResponse, status_code=status.HTTP_201_CREATED)
        ...
        @self.app.post("/books", response_model=BookResponse, status_code=status.HTTP_201_CREATED)
        async def add_book(
            request: BookRequest,
            current_user: User = Depends(self._get_current_user)
        ):
        ...
        async def search_books(
            ...
            if request.search_type == "title":
                ...
                books = list({(book.title, book.author): book for book in title_books + author_books}.values())
        ...
        async def list_members(current_user: User = Depends(self._get_current_user)):
            ...
            return [
                MemberResponse(
                    name=member.name,
                    books_count=len(member.books),
                    books=member.books
                )
                for member in members
            ]
        
        @self.app.post("/members", response_model=MemberResponse, status_code=status.HTTP_201_CREATED)
        ...
        async def issue_book(
            ...
            try:
                self.library.issue_book(request.book_title, request.book_author, request.member_name)
                return {"message": f"Book '{request.book_title}' issued to {request.member_name}"}
            except Exception as e:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
        ...
        async def return_book(
            ...
            try:
                self.library.return_book(request.book_title, request.book_author, request.member_name)
                return {"message": f"Book '{request.book_title}' returned by {request.member_name}"}
            except Exception as e:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
        
        @self.app.get("/circulation/overdue")
        async def get_overdue_books(current_user: User = Depends(self._get_current_user)):
        ...
        async def export_books(
            ...
            if format == ExportFormat.JSON:
                ...
                for book in books:
                    ...
                    p.drawString(72, y, book.title[:25] + "..." if len(book.title) > 25 else book.title)
                    p.drawString(250, y, book.author[:20] + "..." if len(book.author) > 20 else book.author)
                    p.drawString(400, y, book.category)
                    p.drawString(500, y, "Available" if book.due_date is None else "Issued")
                    y -= 15
