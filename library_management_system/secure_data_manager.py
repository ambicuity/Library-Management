...
class SecureDataManager:
    ...
    def __init__(
        ...
        self.encryption = (
            EncryptionManager(encryption_password) if enable_encryption else None
        )
    ...
    def load_books(self) -> List[Book]:
        ...
        try:
            ...
            if self.backup_manager and os.path.exists(self.books_file):
                self.create_backup(
                    f"corrupt_books_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                )
    ...
    def load_members(self) -> List[Member]:
        ...
        try:
            ...
            if self.backup_manager and os.path.exists(self.members_file):
                self.create_backup(
                    f"corrupt_members_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                )
