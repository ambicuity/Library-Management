...
class NotificationManager:
            ...
            try:
                ...
                for name, recipient_data in data.items():
                    self.recipients[name] = NotificationRecipient(
                        ...
                        preferred_method=NotificationType(recipient_data.get("preferred_method", "email"))
                    )
    ...
    def add_webhook(self, url: str, events: List[str], secret: Optional[str] = None) -> None:
    ...
    def send_notification(
        ...
        if not template or not recipient:
            print(f"Template '{template_name}' or recipient '{recipient_name}' not found")
            return False
        ...
        if method == NotificationType.EMAIL and recipient.email:
            ...
            sms_message = f"{subject}\n\n{body}" if len(subject + body) < 160 else body[:157] + "..."
        ...
        if success:
            self._send_webhooks("notification_sent", {
    ...
    def send_system_alert(self, alert_type: str, message: str, severity: str = "normal") -> bool:
        ...
        admin_recipients = [
            name for name, recipient in self.recipients.items()
...
class NotificationScheduler:
    ...
    def _send_due_reminders(self) -> None:
        ...
        for member in self.library.display_members():
            for title, checked_out_book in member.checked_out_books.items():
                try:
                    due_date = datetime.strptime(checked_out_book.due_date, "%Y-%m-%d").date()
        ...
        if books_due_soon:
            sent_count = self.notification_manager.send_due_book_reminders(books_due_soon)
    ...
    def _send_overdue_notices(self) -> None:
        ...
        if overdue_list:
            sent_count = self.notification_manager.send_overdue_book_notices(overdue_list)
    ...
    def _send_system_health_check(self) -> None:
        ...
        self.notification_manager.send_system_alert(
            alert_type="System Health Check",
            message=message,
            severity=severity
        )
