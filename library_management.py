...
def setup_system(
    enable_encryption: bool = True, enable_monitoring: bool = True
) -> tuple:
    ...
    data_manager = SecureDataManager(
        enable_encryption=enable_encryption, enable_backups=True
    )
    ...
    if enable_monitoring:
        monitoring_system = MonitoringSystem(
            enable_performance_monitoring=True, enable_security_monitoring=True
        )

        # Record system startup
        monitoring_system.record_system_event(
            "system_startup", "Library Management System started successfully"
        )
...
def run_cli_mode(
    library: Library, auth_manager: AuthManager, monitoring_system=None
) -> None:
    ...
    cli = CLI(
        library=library, auth_manager=auth_manager, monitoring_system=monitoring_system
    )
    cli.run()
...
def run_combined_mode(
    library: Library, auth_manager: AuthManager, host: str = "127.0.0.1"
) -> None:
    ...
    try:
        ...
        api_thread = threading.Thread(
            target=lambda: api.run(host=host, port=8000, reload=False), daemon=True
        )
        api_thread.start()
...
Examples:
  ...
  python library_management.py --no-monitoring    # Disable monitoring
    ...
    parser.add_argument(
        "--no-encryption", action="store_true", help="Disable data encryption"
    )

    parser.add_argument(
        "--no-monitoring", action="store_true", help="Disable monitoring and logging"
    )
    ...
    try:
        ...
        if monitoring_system:
            monitoring_system.record_system_event(
                "system_shutdown", "Library Management System stopped by user"
            )
        ...
        if "monitoring_system" in locals() and monitoring_system:
            monitoring_system.record_system_event(
                "system_error", f"Fatal error occurred: {e}", level=LogLevel.ERROR
            )
            monitoring_system.shutdown()
